...
class UploadNextcloud extends Upload{
	...
	/**
	 * 设置和获取分享链接，其实主要是设置，因为设置的时候就可以返回分享链接，只不过图片要加个"/preview"才能真正的用于markdown中
	 * 否则如果非图片，则可以加个“/download”直接下载。
	 *
	 * @param $path
	 * @param $action
	 *
	 ...
	 */
	public function shareLink($path, $action='set'){
	    ...
	    //============================ GuzzleHttp start (not working, don't know why) ============================
	    /*$shareBaseUri = 'https://nextcloud-fi.webo.hosting/ocs/v1.php/apps/files_sharing/api/v1/';
		$client = new Client([
			'base_uri' => $shareBaseUri,
		]);
		$path = '/2019/03/31/b6808e0f56c41390e5e0cf8cc8694155.jpeg';
		$uri = 'shares?path='.$path.'&shareType=3&permissions=1';
